...
export default function Home() {
  ...
  const newRecord1 = Record.filter((el) => el.category === "men's clothing");
  const newRecord2 = Record.filter((el) => el.category === "women's clothing");
  ...
  return (
    <>
      ...
      <div className="flex justify-evenly mt-6">
        <button className="border hover:border-[#008ABF]">
          <Link to="/">Home</Link>
        </button>
        <button className="border hover:border-[#008ABF]">
      ...
      <div className="flex justify-evenly mt-10">
        {newRecord1.map((e, i) => (
          <ul key={i} className="w-[350px] h-[550px]">
            <li className="h-[55px] text-center p-3 font-serif font-semibold">
              {e.category}
            </li>
            <hr className="border-2" />
            <br />
            <center>
              <img className="h-[250px]" src={e.image} alt="" />
            </center>
            <br />
            <li className="text-center m-2">
              <button className="w-[170px] m-2 font-semibold" id="btn1">
                <small className="m-3">Price :</small>
                {e.price}
              </button>
            </li>
            <li className="text-center">
              <button className="w-[150px] m-2 font-semibold" id="btn1">
                <small className="m-3">Rate :</small>
                {e.rating.rate}
              </button>
              <button className="w-[150px] m-2 font-semibold" id="btn1">
                <small className="m-3">Count :</small>
                {e.rating.count}
              </button>
            </li>
          </ul>
          ...
          <div className='flex justify-evenly'>
                ...
                <div className='flex justify-evenly'>
            {
                newRecord4 &&
                newRecord4.map((e, i) => {
                    return <ul key={i} className=' h-[550px] w-[350px]' id='ul1'>
                    <li className=' h-[55px] text-center p-3 font-serif font-semibold'>{e.category}</li><hr className='border-2' />
                     <br /><br />
                      <center>
                      <img className='h-[250px] w-[230px]' src={e.image} alt="" />
